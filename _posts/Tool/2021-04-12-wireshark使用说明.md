...
### 帧列表表头右击菜单
<img src="/public/zimage/tool/wireshark/wireshark_column_right_menu.jpg">

#### Edit Column 

可以对当前表头进行调整  比如 把 Arrival Time  起个别名  抵达时间 ( 这样方便查看 )
...
#### Apply As Column 
选中对应帧的字段 邮件 选择 Apply 就可以把当前列作为 标题显示在 帧列表中
例如:  Arrival Time 作为显示列项 可以看到 帧捕获的时间点


#### Copy + As Fliter
可以把当前的选中项作为过滤条件复制到剪切板中 (可以查看当前选中详情 作为过滤项的具体过滤命令)

例如: 
wlan.fc.type_subtype == 0x0008   Beacon 帧
wlan.fc.type_subtype == 0x0028   Qos_Data 帧
...
## View 显示设置菜单
<img src="/public/zimage/tool/wireshark/wireshark_view_menu.jpg">

### View->Time Disply Format

可以选中 时间显示的格式，默认显示的时间是从第一帧开始的毫秒数  可以转为对应的 yyyy-MM-dd HH:mm:ss 这样的显示格式
