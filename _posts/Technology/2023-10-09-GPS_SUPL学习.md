...
IKSWV-27932: Old device's WiFi disconnects while transferring data
新到 传输数据过程中 WIFI 断开的问题 正在下载Log 尽快分析 


IKSWV-26656: DUT cannot turn on MHS
当前Huston 不支持DBS 但当前手机能同时触发 本地正在编译版本排查 去除对 同发 STA 和 AP 的支持 ，本地编译版本验证中。
...
IKSWUDCM-1873: FIT:JP-DoCoMo:LTE:null:F:Y1:N1: The positioning service is performed without using the SUPL server
SUPL测试服务器地址问题 当前已经在48版本完成了服务器地址的更新, 测试人员反馈定位失败 但还未提供Log过来分析 将催促尽快提供Log过来分析

IKSWUDCM-1951
FIT:JP-DoCoMo:When performing SUPL positioning, the APN cannot switch.
设备在进行SUPL通信时需要设置专用的APN,这是一个开关当前测试版本已经打开，怀疑测试时测试人员手动把这个开关关闭了
反馈测试人员在48版本复测 并确保当前 专用的APN 开关是被选中的 , 目前等待测试人员进行一个复测并反馈
